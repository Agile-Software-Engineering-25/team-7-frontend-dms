    ...
    const handleConfirm = () => {
        if (mode === "all") {
            if (!items || items.length === 0) {
                alert(t('documentManagement.snack.noDocsInFolder', 'No documents or folders in current directory'));
                return;
            }
                ...
                <FormControl component="fieldset" fullWidth>
                    ...
                    >
                        <FormControlLabel
                            value="all"
                            control={<Radio />}
                            label={t('documentManagement.downloadDocument.allDocs', 'Download all documents')}
                        />
                        <FormControlLabel
                            value="select"
                            control={<Radio />}
                            label={t('documentManagement.downloadDocument.selectedDocs', 'Choose documents')}
                        />
                    </RadioGroup>
                </FormControl>
                        ...
                        {items.map((doc) => (
                            ...
                            >
                                <ListItemButton onClick={() => toggleItem(doc.id)}>
                                    <ListItemIcon>
                                        <Checkbox edge="start" checked={selected.includes(doc.id)} tabIndex={-1} />
                                    </ListItemIcon>
                                    <ListItemText
                                        primary={doc.name}
